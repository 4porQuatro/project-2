...
class AddPermissionsSeeder extends Seeder
{
    ...
    public function run()
    {
        $permission_group = PermissionGroup::create(['name'=>'order', 'label'=>'Encomendas']);
        $role = $this->getRole();
        foreach($this->methods as $method)
        {
            $permission = Permission::create(['name'=>'order_'.$method['name'], 'label'=>$method['label'], 'group_id'=>$permission_group->id]);
    ...
    {
        $permission_group = PermissionGroup::where(['name' => 'order'])->first();
        $role = $this->getRole();
