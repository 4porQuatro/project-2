<div class="relative bg-white py-16 sm:py-24 lg:py-32">
  <div class="mx-auto max-w-md px-4 text-center sm:max-w-3xl sm:px-6 lg:max-w-7xl lg:px-8">
    @if(!empty($data->title->default))
      <h2 class="text-base font-semibold uppercase tracking-wider text-indigo-600">{{$data->title->default}}</h2>
    @endif
    @if(!empty($data->subtitle->default))
        <p class="mt-2 text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">{{$data->subtitle->default}}</p>
    @endif
    <div class="mt-12">
