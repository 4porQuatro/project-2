    ...
    <div class="lg:grid lg:grid-cols-12">
      ...
      <div class="relative bg-indigo-600 lg:col-start-3 lg:row-start-1 lg:col-span-10 lg:rounded-3xl lg:grid lg:grid-cols-10 lg:items-center">
        ...
        <div class="relative max-w-md mx-auto py-12 px-4 space-y-6 sm:max-w-3xl sm:py-16 sm:px-6 lg:max-w-none lg:p-0 lg:col-start-4 lg:col-span-6">
          <h2 class="text-3xl font-extrabold text-white" id="join-heading">{{$data->title->default}}</h2>
          @if(!empty($data->text->default))
            <p class="text-lg text-white">{{$data->text->default}}</p>
          @endif
            @if(!empty($data->button_link->default))
                <a href="{{$data->button_link->default}}" class="block w-full py-3 px-5 text-center bg-white border border-transparent rounded-md shadow-md text-base font-medium text-indigo-700 hover:bg-gray-50 sm:inline-block sm:w-auto">{{$data->button_title->default}}</a>
            @endif
        </div>
      </div>
    </div>
