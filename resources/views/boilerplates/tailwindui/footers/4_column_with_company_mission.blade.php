      ...
      <div class="mt-12 grid grid-cols-2 gap-8 xl:mt-0 xl:col-span-2">
        <div class="md:grid md:grid-cols-2 md:gap-8">
          <div>
            ...
            <ul role="list" class="mt-4 space-y-4">
                @foreach($data->article_1->default as $article)
                    <li>
          ...
          <div class="mt-12 md:mt-0">
            ...
            <ul role="list" class="mt-4 space-y-4">
                @foreach($data->article_2->default as $article)
                    <li>
                        <a href="{{$article->link}}" class="text-base text-gray-500 hover:text-gray-900">{{$article->title}}</a>
                    </li>
                @endforeach
        ...
        <div class="md:grid md:grid-cols-2 md:gap-8">
          <div>
            ...
            <ul role="list" class="mt-4 space-y-4">
                @foreach($data->article_3->default as $article)
                    <li>
                        <a href="{{$article->link}}" class="text-base text-gray-500 hover:text-gray-900">{{$article->title}}</a>
                    </li>
                @endforeach
          ...
          <div class="mt-12 md:mt-0">
            <h3 class="text-sm font-semibold text-gray-400 tracking-wider uppercase">{{$data->text_4->default}}</h3>
            <ul role="list" class="mt-4 space-y-4">
                @foreach($data->article_4->default as $article)
                    <li>
                        <a href="{{$article->link}}" class="text-base text-gray-500 hover:text-gray-900">{{$article->title}}</a>
                    </li>
                @endforeach
